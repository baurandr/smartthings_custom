...
metadata {
	definition (name: "GE Motion Dimmer Switch 26933", namespace: "baurandr", author: "Andrew Baur") {
...
def zwaveEvent(physicalgraph.zwave.commands.configurationv2.ConfigurationReport cmd) {
    ...
    if (cmd.parameterNumber == 1) {
    	...
    	def value = config == 0 ? "Disabled" : config == 1 ? "10 sec" : config == 2 ? "20 sec" : config == 3 ? "30 sec" : config == 4 ? "45 sec" : config == 5 ? "60 sec" : config == 6 ? "75 sec" : "27 minute" 
...
def on() {
    delayBetween([zwave.basicV1.basicSet(value: 0xFF).format(), zwave.switchMultilevelV3.switchMultilevelGet().format()], 3000) 
}


def off() {
	delayBetween ([zwave.basicV1.basicSet(value: 0x00).format(), zwave.switchMultilevelV3.switchMultilevelGet().format()], 3000)
}
